...
func (s *server) DeleteFriend(ctx context.Context, req *usr.DeleteFriendRequest) (*usr.DeleteFriendResponse, error) {
	if req == nil {
		return nil, errors.New("req is empty")
	}
	if req.UserName == "" || req.DeletedUserName == "" {
		return nil, errors.New("username or added username is invalid")
	}
	//FIXME：需要校验吗？
	_, err := CheckUserInDB([]string{req.UserName, req.DeletedUserName})
	if err != nil {
		return nil, err
	}
	...
	relation := model.UserRelation{
		...
		RelationUserName: req.DeletedUserName,
		RelationType:     int(model.DELETED),
	}
	...
	return &usr.DeleteFriendResponse{ErrorCode: usr.UserErrorCode_OK}, nil
}
